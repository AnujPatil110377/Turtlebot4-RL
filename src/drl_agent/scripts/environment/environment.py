...
class Environment(Node):
    ...
    def __init__(self):
        ...
        self.odom
        # LaserScan subscription (replace Velodyne)
        ...
        self.laser
        # Define bins for grouping the laser scan data
    ...
    def update_environment_state(self, scan_msg):
        """
        Updates the environment state using 2D LaserScan data.
        The scan_msg.ranges is a 1D array of distances (meters) at regular angular intervals.
        We divide the scan into bins (e.g., environment_dim bins for 180 degrees), and for each bin,
        we take the minimum distance (closest obstacle).
        """
        with self.environment_state_lock:
            ranges = np.array(scan_msg.ranges)
            # Replace inf/nan with max range
            valid_ranges = np.where(
                np.isfinite(ranges), ranges, scan_msg.range_max
            )
            # Divide the scan into bins
            bins = np.array_split(valid_ranges, self.environment_dim)
            # For each bin, take the minimum distance (closest obstacle)
            state = np.array([np.min(b) for b in bins])
            # Optionally normalize or clip
            state = np.clip(state, scan_msg.range_min, scan_msg.range_max)
